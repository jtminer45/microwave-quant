...
sys.path.append('src')
from data_fetcher import get_ngx_prices, get_stock_volatility
...
st.sidebar.warning("""
⚠️ **Disclaimer**
This dashboard uses delayed market data. All information is for 
analytical purposes only and does not constitute investment advice. 
Past performance does not guarantee future results.
© 2026 Microwave Quant
""")
...
if page == "🏠 Market Overview":
    ...
    if df is not None:
        ...
        with col1:
            ...
            gainers_df.columns = ['Ticker', 'Price (NGN)', 'Change %']
            st.dataframe(gainers_df, use_container_width=True, hide_index=True)

        with col2:
            st.subheader("📉 Top 5 Losers")
            losers_df = df.nsmallest(5, 'change_pct')[['ticker', 'price', 'change_pct']]
            losers_df.columns = ['Ticker', 'Price (NGN)', 'Change %']
        ...
        display_df.columns = ['Ticker', 'Company', 'Price (NGN)', 'Change %', 'Market Cap']
        ...
        st.error("Could not fetch NGX data. Please check your internet connection.")
...
# ── Stock Analysis ──
    ...
    if df is not None:
        ...
        hist_file = f"/Users/minerjonathan/Desktop/ngx-predictor/data/{ticker}.csv"

        if os.path.exists(hist_file):
            hist_df = pd.read_csv(hist_file)
            hist_df['Date'] = pd.to_datetime(hist_df['Date'], format='mixed', dayfirst=True)
            hist_df = hist_df.sort_values('Date')
            hist_df['Price'] = pd.to_numeric(
                hist_df['Price'].astype(str).str.replace(',', ''), errors='coerce'
            )
            ...
            with col2:
                ...
                st.metric("52W High", f"NGN{hist_df['Price'].max():,.2f}")
                st.metric("52W Low", f"NGN{hist_df['Price'].min():,.2f}")
        else:
            st.warning(f"No historical data available for {ticker}")
            st.info("Historical data available for: DANGCEM, MTNN, GTCO, ZENITHBANK, SEPLAT, UBA, ACCESSCORP, AIRTELAFRI, BUACEM, NESTLE")
...
# ── Portfolio Analytics ──
    ...
    if df is not None:
        ...
        for i, stock in enumerate(st.session_state.portfolio_stocks):
            ...
            with col3:
                buy_px = st.number_input("Buy Price (NGN)", min_value=0.01,
        ...
        for s in st.session_state.portfolio_stocks:
            ...
            portfolio.append({
                "Ticker": s['ticker'],
                "Quantity": s['qty'],
                "Buy Price (NGN)": s['buy_price'],
                "Current Price (NGN)": current_price,
                "Cost (NGN)": round(cost, 2),
                "Value (NGN)": round(value, 2),
                "P&L (NGN)": round(pnl, 2),
                "Return %": round(return_pct, 2)
            })

        portfolio_df = pd.DataFrame(portfolio)
        total_cost = portfolio_df['Cost (NGN)'].sum()
        total_value = portfolio_df['Value (NGN)'].sum()
        ...
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("Expected Value", f"NGN{final_values.mean():,.2f}")
        col2.metric("Best Case (95th)", f"NGN{np.percentile(final_values, 95):,.2f}")
        col3.metric("Worst Case (5th)", f"NGN{np.percentile(final_values, 5):,.2f}")
        ...
        fig.update_layout(title="Portfolio Monte Carlo Simulation",
                         ...
                         height=400)
        st.plotly_chart(fig, use_container_width=True)
        # ASI Benchmark Comparison
        st.markdown("---")
        st.subheader("📊 Portfolio vs ASI Benchmark")
        
        # ASI YTD return (NGX All Share Index)
        asi_ytd = 32.32  # ASI YTD return as of July 2026
        ...
        if total_return > asi_ytd:
            st.success(f"✅ Your portfolio is outperforming the ASI benchmark by {total_return - asi_ytd:.2f} percentage points")
        else:
            st.warning(f"⚠️ Your portfolio is underperforming the ASI benchmark by {asi_ytd - total_return:.2f} percentage points")
        ...
        if st.button("📥 Generate PDF Report"):
            ...
            holdings_data = [['Ticker', 'Qty', 'Buy Price', 
                             'Current', 'P&L', 'Return %']]
            for p in portfolio:
                holdings_data.append([
                    p['Ticker'],
                    f"{p['Quantity']:,}",
                    f"₦{p['Buy Price (NGN)']:,.2f}",
                    f"₦{p['Current Price (NGN)']:,.2f}",
                    f"₦{p['P&L (NGN)']:,.2f}",
                    f"{p['Return %']:.2f}%"
                ])
            
            holdings_table = Table(holdings_data, 
                                  colWidths=[1*inch, 0.8*inch, 1.2*inch, 
                                            1.2*inch, 1.2*inch, 0.8*inch])
            ...
            # Monte Carlo
            story.append(Paragraph("Monte Carlo Simulation — 1,000 Scenarios", 
                                  styles['Heading2']))
            ...
            story.append(Paragraph(
                ...
                "© 2026 Microwave Quant — Built by Jonathan Miner",
                body_style
            ))
...
# ── Derivatives Pricing ──
    ...
    if df is not None:
        ...
        with col1:
            ...
            st.metric("Current Price", f"NGN{S:,.2f}")

            K = st.number_input("Strike Price (NGN)",
                               value=float(round(S * 1.1, 2)),
                               step=10.0)
            T = st.slider("Time to Expiry (months)", 1, 12, 3) / 12
            r = st.slider("Risk-Free Rate (%)", 10, 35, 27) / 100

            # Real volatility per stock
            real_vol = get_stock_volatility(ticker)
            sigma = st.slider("Volatility (%)", 10, 100,
                             int(real_vol * 100)) / 100
        ...
        with col2:
            ...
            col_a.metric("Option Price", f"NGN{price:,.2f}")
            col_b.metric("Break-even", f"NGN{break_even:,.2f}")
            ...
            fig.update_layout(title=f"{ticker} Delta Curve",
                            xaxis_title="Stock Price (NGN)",
                            yaxis_title="Delta",
                            height=350)
            st.plotly_chart(fig, use_container_width=True)

            # THE DERIVATIVE SCANNER HERE
            st.markdown("---")
            ...
            for _, row in df.iterrows():
                ...
                try:
                    ...
                    scanner_results.append({
                        "Ticker": row['ticker'],
                        "Price (NGN)": f"NGN{s:,.2f}",
                        "Strike (NGN)": f"NGN{k:,.2f}",
                        "Call (NGN)": f"NGN{call_price:,.2f}",
                        "Put (NGN)": f"NGN{put_price:,.2f}",
                        "Delta": round(d, 4),
                        "IV": f"{stock_vol*100:.1f}%",
                        "Break-even (NGN)": f"NGN{break_even:,.2f}",
                        "Signal": signal
                    })
                except:
                    continue
...
# ── Risk Dashboard ──
    ...
    if df is not None:
        ...
        with col2:
            portfolio_value = st.number_input("Portfolio Value (NGN)",
                                             value=10_000_000, step=1_000_000)
        with col3:
            confidence = st.slider("Confidence Level (%)",
                                  min_value=90, max_value=99, value=95)
            sigma_annual = st.slider("Annual Volatility (%)",
                                    min_value=10, max_value=100, value=45) / 100
            holding_period = st.slider("Holding Period (days)",
        ...
        if selected:
            ...
            col1, col2, col3, col4 = st.columns(4)
            col1.metric("Portfolio Value", f"NGN{portfolio_value:,.2f}")
            col2.metric(f"VaR ({holding_period}d)", f"NGN{var:,.2f}",
                       delta=f"-{var/portfolio_value*100:.2f}%",
                       delta_color="inverse")
            col3.metric(f"CVaR ({holding_period}d)", f"NGN{cvar:,.2f}",
                       delta=f"-{cvar/portfolio_value*100:.2f}%",
                       delta_color="inverse")
            col4.metric("Volatility", f"{sigma_annual*100:.1f}%")

            st.info(f"""
            **Interpretation:**
            - **VaR {confidence}%**: There is a {100-confidence}% chance of losing more than NGN{var:,.2f} over {holding_period} day(s)
            - **CVaR {confidence}%**: If losses exceed VaR, the average loss will be NGN{cvar:,.2f}
            ...
            vars = [portfolio_value * sigma_period * norm.ppf(c/100)
                   for c in confidence_levels]
            cvars = [portfolio_value * sigma_period * norm.pdf(norm.ppf(c/100)) / (1-c/100)
                    for c in confidence_levels]

            fig = go.Figure()
            fig.add_trace(go.Scatter(x=confidence_levels, y=vars,
                                    name='VaR', line=dict(color='orange', width=2)))
            fig.add_trace(go.Scatter(x=confidence_levels, y=cvars,
            ...
            for scenario, impact in scenarios.items():
                ...
                stress_results.append({
                    ...
                    "New Value (NGN)": f"NGN{stressed_value:,.2f}",
                    "P&L (₦)": f"NGN{pnl:,.2f}",
            ...
            n_options = st.number_input("Number of Call Options",
            ...
            col3.metric("Hedge Cost (NGN)", f"NGN{hedge_cost:,.2f}")
    ...
    # Available stocks with historical data
    available_stocks = {
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        bt_ticker = st.selectbox("Select Stock", list(available_stocks.keys()),
                                index=4)  # Default SEPLAT
    with col2:
    ...
    hist_file = f"/Users/minerjonathan/Desktop/ngx-predictor/data/{bt_ticker}.csv"

    if os.path.exists(hist_file):
        df_bt = pd.read_csv(hist_file)
        df_bt['Date'] = pd.to_datetime(df_bt['Date'], format='mixed', dayfirst=True)
        df_bt = df_bt.sort_values('Date').reset_index(drop=True)
        df_bt['Price'] = pd.to_numeric(
            df_bt['Price'].astype(str).str.replace(',', ''), errors='coerce'
        )
        df_bt = df_bt.dropna(subset=['Price'])

        # ── Strategy Logic ──
        if strategy == "Buy and Hold":
            df_bt['Signal'] = 1  # Always long
            df_bt['Position'] = 1
            ...
            df_bt['Signal'] = 0
            df_bt.loc[df_bt['MA20'] > df_bt['MA50'], 'Signal'] = 1   # Buy
            df_bt.loc[df_bt['MA20'] < df_bt['MA50'], 'Signal'] = -1  # Sell
            ...
            df_bt['Signal'] = 0
            df_bt.loc[df_bt['RSI'] < 30, 'Signal'] = 1   # Oversold — Buy
            df_bt.loc[df_bt['RSI'] > 70, 'Signal'] = -1  # Overbought — Sell
            df_bt['Position'] = df_bt['Signal']

        elif strategy == "Delta Signal (Options Based)":
            # Use our Black-Scholes Delta signal
            from scipy.stats import norm
            r_bt = 0.265
            ...
            deltas = []
            for price in df_bt['Price']:
                try:
                    k = price * 1.10
                    d1 = (np.log(price/k) + (r_bt + sigma_bt**2/2) * T_bt) / \
            ...
            df_bt['Delta'] = deltas
            df_bt['Signal'] = 0
            df_bt.loc[df_bt['Delta'] > 0.5, 'Signal'] = 1   # BUY CALL
            df_bt.loc[df_bt['Delta'] < 0.3, 'Signal'] = -1  # BUY PUT
            df_bt['Position'] = df_bt['Signal']
        ...
        # ── Key Metrics ──
        total_return = (df_bt['Portfolio_Value'].iloc[-1] - initial_capital) / initial_capital * 100
        bh_return = (df_bt['BuyHold_Value'].iloc[-1] - initial_capital) / initial_capital * 100
        
        # Sharpe Ratio
        sharpe = df_bt['Strategy_Return'].mean() / df_bt['Strategy_Return'].std() * np.sqrt(252)
        ...
        total_days = len(df_bt['Strategy_Return'])
        ...
        col1.metric("Strategy Return", f"{total_return:.2f}%",
                   delta=f"{total_return - bh_return:.2f}% vs Buy & Hold")
        ...
        if strategy != "Buy and Hold":
            ...
            fig_sig.update_layout(
                title=f"{bt_ticker} Price with Trading Signals",
                xaxis_title="Date",
        ...
        summary = pd.DataFrame({
            'Metric': [
                'Strategy', 'Stock', 'Period',
                'Initial Capital', 'Final Value', 'Total Profit',
                'Strategy Return', 'Buy & Hold Return', 'Alpha',
                'Sharpe Ratio', 'Max Drawdown', 'Win Rate', 'Trading Days'
            ],
            'Value': [
                strategy, available_stocks[bt_ticker],
        ...
        Past performance does not guarantee future results. 
        Backtested results do not account for transaction costs, 
        slippage, liquidity constraints, or market impact. 
        This analysis is for educational purposes only.
        """)

    else:
        st.warning(f"No historical data found for {bt_ticker}")
        st.info("Please ensure CSV files are in: /Users/minerjonathan/Desktop/ngx-predictor/data/")